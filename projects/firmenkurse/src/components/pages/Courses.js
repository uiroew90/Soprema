...
const Category = ({ category, courses, selectedCourses, toggleCourse }) => {
  ...
  const countSelectedCourses = () => {
    return courses?.filter((course) => selectedCourses?.some((selectedCourse) => selectedCourse.name === course.name && selectedCourse.category === course.category)).length || 0;
  };
...
export default ({ courses, selectedCourses, setSelectedCourses }) => {
  ...
  return (
    <RouteWrapper hasCallout="true" breadcrumbLinks={[crumb.home, crumb.packs, crumb.current]} titleText={t("routeCourses.title")}>
