...
export default ({ selectedCourses, handleRemoveCourse, handleCourseComment }) => {
  ...
  const crumb = {
    ...
    current: { text: t("breadcrumbs.overview") },
  };

  return (
    <RouteWrapper breadcrumbLinks={[crumb.home, crumb.packs, crumb.courses, crumb.current]} titleText={t("routeOverview.title")}>
                          ...
                          <div className="small-tule-container">
                            <div className="small-tule-content cart-listing__item__content std-form wysiwyg mb-0">
                              ...
                              <button className="button" onClick={() => handleRemoveCourse(course)}>
                                <svg style={{ aspectRatio: "1", verticalAlign: "-0.125em", width: "1em" }} aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
            ...
            <div className="col col-sm-12">
              {selectedCourses.length === 0 && (
                <div className="wysiwyg">
                  <h2>{t("routeOverview.fallbackHeader")}</h2>
                  <p>
                    <Trans i18nKey="routeOverview.fallbackMessage">
                      Please go back to <Link to="/packs">Packs</Link> or <Link to="/courses">Courses</Link> before proceeding to checkout.
                    </Trans>
                  </p>
                </div>
              )}
              {selectedCourses.length !== 0 && (
                <div className="wysiwyg text-right">
                  <button onClick={() => selectedCourses.length > 0 && navigate("/checkout")} disabled={selectedCourses.length === 0} className="button">
                    {t("routeOverview.buttonText")}
                  </button>
                </div>
              )}
            </div>
