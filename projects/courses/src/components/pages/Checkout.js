...
export default ({ selectedCourses, setSelectedCourses }) => {
  ...
  useEffect(() => {
    ...
    const selectedCoursesText = selectedCourses
      .map((course) => `• ${course.name}${course.comment ? `\n  "${course.comment}"` : ""}`)
      .join("\n");
      ...
      <div className='mega-row cms-row vertical-spacing-bottom'>
        <div className='container container-medium'>
          <div className='row'>
            ...
            <div className='col col-sm-12'>
              <form
                name='form-course-order'
                method='post'
                action='https://go.soprema.ch/l/978613/2023-02-24/594nlk'
                className='std-form mb-0'
